...
bool CFbxMesh::LoadAnimation(const TCHAR* FName, const ROOTANIMTYPE& RAType)
{
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		// ルートボーンに関するデータ
		memcpy_s(&m_Animation[m_allAnimationCount].RootBoneMesh, sizeof(int), p, sizeof(int));       // ルートボーンメッシュ番号を得る    // -- 2020.12.15 -- 3
		p += sizeof(int);
		memcpy_s(&m_Animation[m_allAnimationCount].RootBone, sizeof(int), p, sizeof(int));           // ルートボーン番号を得る            // -- 2020.12.15 -- 3
		p += sizeof(int);
