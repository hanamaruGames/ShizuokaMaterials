...
bool CBBox::OBBCollisionLay( const VECTOR3&  vNow, const VECTOR3& vOld, VECTOR3* vHit, VECTOR3* vNrm)
{
	...
	for (i = 0; i<12; i++)
	{
		...
		// 面法線を作成する
		//vFaceNorm = normalize(cross(vVert[1] - vVert[0], vVert[2] - vVert[0]));   // 右回りを表とする
		vFaceNorm = normalize(cross(vVert[2] - vVert[0], vVert[1] - vVert[0]));     // 左回りを表とする  // -- 2021.1.11
